<x-layouts.admin :title="__('Storage')" admin-key="storage">
    ...
    <main class="px-4 sm:px-7 py-7 space-y-5">
        ...
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                ...
                <h1 class="font-serif font-normal tracking-[-0.01em] text-[28px] sm:text-[40px] leading-[1.0]">
                    {{ __('Cloud') }} <span class="italic ig-text">{{ __('storage') }}</span>.</h1>
        ...
        @endif
        @if (session('error'))
            <div class="rounded-2xl border border-accent-coral/30 bg-accent-coral/10 text-accent-coral px-4 py-2 text-[12.5px]">{{ session('error') }}</div>
        @endif
                ...
                <section class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card overflow-hidden">
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <label class="space-y-1.5 block border-t border-paper-200 pt-4"><span class="text-[11.5px] font-semibold">{{ __('Base folder path') }} <span class="text-ink-500 font-normal">· {{ __('optional') }}</span></span>
                            <input name="base_path" value="{{ old('base_path', $cfg['base_path'] ?? '') }}" placeholder="instaflow" class="w-full rounded-xl border border-paper-200 bg-paper-0 px-3 py-2.5 text-[13px] font-mono focus:outline-none focus:border-wa-deep">
