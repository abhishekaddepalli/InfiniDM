{{--
 /admin/system-health — one screen for "is everything up?": database, cache,
 queue, storage, runtime, connected Instagram accounts and host vitals. Polls
 ?format=json every 20s to stay live. Charts/gauges are inline SVG/CSS.
--}}
<x-layouts.admin :title="__('System Health')" admin-key="health">

    @php
        $tone = [
            'up'   => ['dot' => 'bg-wa-green',    'text' => 'text-wa-deep',      'bg' => 'bg-wa-mint',          'ring' => 'ring-wa-green/15',    'word' => __('Operational')],
            'warn' => ['dot' => 'bg-accent-amber', 'text' => 'text-accent-amber', 'bg' => 'bg-accent-amber/10',  'ring' => 'ring-accent-amber/15', 'word' => __('Degraded')],
            'down' => ['dot' => 'bg-accent-coral', 'text' => 'text-accent-coral', 'bg' => 'bg-accent-coral/10',  'ring' => 'ring-accent-coral/15', 'word' => __('Down')],
    ...
    <main class="px-4 sm:px-7 py-5 space-y-5">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            @php
                $acts = [
                    ...
                    ['label' => __('AI replies · 24h'),   'value' => $activity['ai_replies'] ?? 0,    'icon' => '<circle cx="8" cy="8" r="2.4"/><path d="M8 2v2M8 12v2M2 8h2M12 8h2"/>'],
                    ['label' => __('Active chats · 24h'), 'value' => $activity['conversations'] ?? 0, 'icon' => '<path d="M2 4h12v7H5l-3 2V4Z"/>'],
                    ['label' => __('Automations live'),   'value' => $activity['automations'] ?? 0,   'icon' => '<path d="M9 1.5 3 9h4l-1 5.5 6-7.5H8z"/>'],
                ];
            @endphp
            @foreach ($acts as $a)
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                @foreach ($checks as $key => $c)
                    ...
                    <div class="bg-paper-0 border border-paper-200 rounded-2xl p-4 shadow-card" data-hk-card="{{ $key }}">
                        ...
                        <div class="font-serif text-[19px] text-ink-900 mt-2.5" data-hk-value>{{ $c['value'] }}</div>
                        <div class="text-[11.5px] text-ink-500 mt-1 break-words" data-hk-detail>{{ $c['detail'] }}</div>
                    </div>
                @endforeach
            </div>
        </div>
                    ...
                    <svg viewBox="0 0 {{ $TW }} {{ $TH }}" preserveAspectRatio="none" class="w-full h-40">
                        <defs>
                            ...
                            <linearGradient id="hbFill" x1="0" y1="0" x2="0" y2="1">
                                <stop offset="0%" stop-color="#E1306C" stop-opacity="0.20" />
                                <stop offset="100%" stop-color="#E1306C" stop-opacity="0" />
                            </linearGradient>
                        </defs>
                        @if ($tarea)<polygon points="{{ $tarea }}" fill="url(#hbFill)" />@endif
                        <polyline points="{{ $tline }}" fill="none" stroke="url(#hbStroke)" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round" vector-effect="non-scaling-stroke" />
                    </svg>
                @else
        ...
        {{-- Connected accounts --}}
        <div>
            <div class="font-mono text-[10px] uppercase tracking-[0.16em] text-ink-500 mb-3">{{ __('Instagram accounts & channels') }}</div>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                @forelse ($engines as $e)
                    ...
                    <div class="bg-paper-0 border border-paper-200 rounded-2xl p-4 shadow-card">
                        ...
                        <div class="text-[11.5px] text-ink-500 mt-1">{{ __('connected of total accounts') }}</div>
                ...
                @empty
                    <div class="text-[12.5px] text-ink-400 py-4">{{ __('No Instagram accounts connected yet.') }}</div>
                @endforelse
            </div>
        </div>
            ...
            @if (isset($system['disk_used_pct']))
                <div class="mt-4">
                    ...
                    <div class="h-2 rounded-full bg-paper-100 overflow-hidden">
                        <div class="h-full rounded-full {{ $system['disk_used_pct'] > 90 ? 'bg-accent-coral' : ($system['disk_used_pct'] > 75 ? 'bg-accent-amber' : 'ig-grad-soft') }}" style="width: {{ $system['disk_used_pct'] }}%"></div>
                    </div>
                </div>
            @endif
        </div>
    ...
    <script>
        (function () {
            var TONE = {
                up:   { dot: 'bg-wa-green',    text: 'text-wa-deep',      bg: 'bg-wa-mint',         word: @json(__('Operational')) },
                warn: { dot: 'bg-accent-amber', text: 'text-accent-amber', bg: 'bg-accent-amber/10', word: @json(__('Degraded')) },
                down: { dot: 'bg-accent-coral', text: 'text-accent-coral', bg: 'bg-accent-coral/10', word: @json(__('Down')) },
                idle: { dot: 'bg-ink-300',      text: 'text-ink-500',      bg: 'bg-paper-100',       word: @json(__('Idle')) },
            };
            var url = @json(route('admin.health')) + '?format=json';
