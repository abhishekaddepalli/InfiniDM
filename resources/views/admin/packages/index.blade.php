<x-layouts.admin :title="__('Admin · Packages')" admin-key="packages">
    ...
    <main class="px-4 sm:px-7 py-7 space-y-5">
        ...
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                ...
                <h1 class="font-serif font-normal tracking-[-0.01em] text-[28px] sm:text-[40px] leading-[1.0]">{{ __('Subscription') }}
                    <span class="italic ig-text">{{ __('packages') }}</span></h1>
                <p class="text-[13px] text-ink-600 mt-2 max-w-2xl">
                    {{ __('Define every plan offered to customers — pricing, account & flow caps, feature flags, and add-ons.') }}
                </p>
            </div>
        ...
        <section class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="bg-paper-0 border border-paper-200 rounded-2xl p-4 shadow-card">
                ...
                <div class="text-[11px] text-ink-500 mt-2">{{ $stats['trial'] }} {{ __('trial') }} · {{ $stats['archived'] }}
                    {{ __('archived') }}</div>
            </div>
            <div class="bg-paper-0 border border-wa-green/40 rounded-2xl p-4 shadow-card">
                <div class="text-[11px] text-ink-600 font-medium">{{ __('Customers subscribed') }}</div>
            ...
            <div class="bg-paper-0 border border-paper-200 rounded-2xl p-4 shadow-card">
                ...
                <div class="font-serif text-[34px] leading-none mt-1">{{ $stats['currency'] }} {{ number_format($stats['mrr'], 2) }}</div>
        ...
        <section class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            @forelse ($packages as $p)
                @php
                    $count = (int) ($p->users_count ?? 0);
                    $free = (float) $p->price <= 0;
                    $bullets = [];
                    $bullets[] = ($p->max_accounts ?: '∞') . ' ' . __('accounts');
                    $bullets[] = ($p->max_flows ?: '∞') . ' ' . __('flows') . ' · ' . ($p->max_automations ?: '∞') . ' ' . __('automations');
                    if ($p->monthly_dms) {
                        $bullets[] = number_format($p->monthly_dms) . ' ' . __('DMs / mo');
                    }
                    foreach (array_slice((array) $p->features, 0, 2) as $f) {
                        $bullets[] = \App\Models\Package::FEATURES[$f] ?? ucfirst($f);
                    }
                @endphp
                <div
                    class="bg-paper-0 border {{ $p->is_featured ? 'border-2 border-wa-deep' : 'border-paper-200' }} rounded-2xl p-5 shadow-card relative">
                    @if ($p->is_featured)
                        <span
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        @else
                            <span
                                class="px-2 py-0.5 rounded-full bg-wa-bubble text-wa-deep text-[10px] font-semibold">{{ ucfirst($p->interval ?? 'month') }}</span>
                        @endif
                    </div>
                    <h3 class="font-serif text-[26px] leading-none">{{ $p->name }}</h3>
                    <div class="mt-3 flex items-baseline gap-1">
                        @if ($free)
                            <span class="font-serif text-[32px]">{{ __('Free') }}</span>
                        @else
                            <span class="font-serif text-[32px]">{{ $p->currency }} {{ number_format((float) $p->price, 2) }}</span>
                            <span class="text-[12px] text-ink-500">/ {{ $p->interval ?? 'month' }}</span>
                        @endif
                    </div>
                    ...
                    <div
                        ...
                        <span class="{{ $count > 50 ? 'text-wa-deep font-semibold' : '' }}">{{ $count }}
                            {{ $count === 1 ? __('customer') : __('customers') }}</span>
                        <a href="{{ route('admin.packages.edit', $p->id) }}" class="text-wa-deep hover:underline">{{ __('Edit') }}
                            →</a>
                    </div>
                </div>
            @empty
                <div class="col-span-full text-center text-ink-500 py-10">
                    {{ __('No packages defined yet.') }} <a href="{{ route('admin.packages.create') }}"
                ...
                <tbody class="divide-y divide-paper-200">
                    @forelse ($packages as $p)
                        @php
                            $count = (int) ($p->users_count ?? 0);
                            $free = (float) $p->price <= 0;
                            $mrrRow = $free ? 0 : (float) $p->price * $count;
                            ...
                            <td class="px-2 py-2">
                                <div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-[10px] text-ink-500 mt-1 font-mono truncate max-w-[240px]">
                                            {{ $p->description ?: __('no subtitle') }}</div>
                                    </div>
                                </div>
                            </td>
                                ...
                                <div class="relative inline-block" data-row-menu>
                                    ...
                                    <div data-row-menu-panel
                                        ...
                                        <form action="{{ route('admin.packages.destroy', $p->id) }}" method="POST"
                                            onsubmit="return confirm('{{ __('Delete') }} {{ addslashes($p->name) }}? {{ __('This cannot be undone.') }}')">
                                            @csrf @method('DELETE')
                                            <button type="submit"
