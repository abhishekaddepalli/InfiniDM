<x-layouts.admin :title="__('Updater')" admin-key="update">
        ...
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                ...
                <h1 class="font-serif font-normal tracking-[-0.01em] text-[28px] sm:text-[40px] leading-[1.0]">
                    {{ __('Application') }} <span class="italic ig-text">{{ __('updater') }}</span>.</h1>
            ...
            <div class="flex items-center gap-2 shrink-0 pb-1">
                ...
                <span class="px-3 py-2 rounded-full bg-paper-100 text-ink-500 text-[11px] font-mono">PHP {{ $phpVersion }} · Laravel {{ $laravel }}</span>
            </div>
        </div>
            ...
            <div class="p-5 space-y-2">
                ...
                <div class="flex items-center justify-between rounded-2xl border border-paper-200 p-3">
                    ...
                    <button type="button" data-rollback="{{ $backup['path'] ?? '' }}" data-version="{{ $backup['version'] ?? '' }}"
                        class="px-4 py-2 rounded-full border border-accent-coral/40 text-accent-coral text-[12px] font-semibold hover:bg-accent-coral/10">{{ __('Rollback') }}</button>
                </div>
                @endforeach
            </div>
        </section>
    ...
    <script src="{{ asset('admin/updater.js') }}?v={{ $currentBuild }}"></script>
</x-layouts.admin>
