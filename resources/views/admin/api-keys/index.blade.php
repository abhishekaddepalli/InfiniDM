<x-layouts.admin :title="__('AI keys')" admin-key="api-keys" page="admin-api-keys-index">
    ...
    <main class="px-4 sm:px-6 lg:px-7 py-7 space-y-5">
        ...
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div>
                ...
                <h1 class="font-serif font-normal tracking-[-0.01em] text-[30px] sm:text-[34px] lg:text-[40px] leading-[1.0]">AI
                    <span class="ig-text italic">{{ __('API keys') }}</span></h1>
                <p class="text-[13px] text-ink-600 mt-2 max-w-3xl">
                    {{ __('Global keys for every AI provider :app supports. Users who bring their own key use theirs; everyone else falls back to these. Encrypted at rest.', ['app' => site_name()]) }}
                </p>
            </div>
        </div>

        @if (session('success'))
            <div class="rounded-2xl border border-wa-green/40 bg-wa-mint text-wa-deep px-4 py-2 text-[12.5px]">
                {{ session('success') }}</div>
        @endif
        @if (session('error'))
            <div class="rounded-2xl border border-accent-coral/30 bg-accent-coral/10 text-[#A1431F] px-4 py-2 text-[12.5px]">
                {{ session('error') }}</div>
        @endif
        ...
        <section class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div class="bg-paper-0 border {{ $stats['no_key'] > 0 ? 'border-accent-amber/40' : 'border-paper-200' }} rounded-2xl p-4 shadow-card">
                <div class="text-[11px] text-ink-600 font-medium">{{ __('No key set') }}</div>
                <div class="font-serif text-[34px] leading-none mt-1 {{ $stats['no_key'] > 0 ? 'text-accent-amber' : '' }}">
                    {{ $stats['no_key'] }}</div>
                ...
                <div class="px-5 py-4 border-b border-paper-200 flex items-center gap-3 flex-wrap">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <span class="text-[10px] font-mono uppercase px-2 py-0.5 rounded-full {{ $p->is_active ? 'bg-wa-mint text-wa-deep border border-wa-green/40' : 'bg-paper-100 text-ink-600' }}">
                                {{ $p->is_active ? 'Active' : 'Disabled' }}
                            </span>
                            @if (!empty($p->api_key))
                                <span class="text-[10px] font-mono uppercase px-2 py-0.5 rounded-full bg-paper-50 text-ink-700 border border-paper-200">{{ __('Key saved') }}</span>
                            @else
                                <span class="text-[10px] font-mono uppercase px-2 py-0.5 rounded-full bg-accent-amber/15 text-accent-amber">{{ __('No key') }}</span>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        <form method="POST" action="{{ route('admin.api-keys.toggle', $p->id) }}" class="inline">
                            @csrf
                            <button class="px-3 py-1.5 rounded-full border {{ $p->is_active ? 'border-paper-200 hover:bg-paper-50' : 'border-wa-green/40 bg-wa-mint text-wa-deep hover:bg-wa-mint/70' }} text-[11.5px] font-semibold">{{ $p->is_active ? 'Disable' : 'Activate' }}</button>
                        </form>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('admin.api-keys.update', $p->id) }}" class="p-5 space-y-4">
                    ...
                    @if (!empty($p->fields_schema))
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach ($p->fields_schema as $key => $spec)
                                @php
                                    $existingVal = $key === 'api_key'
                                        ? ''
                                        : ($p->extra_config_decoded[$key] ?? '');
                                    $hasApiKey = $key === 'api_key' && !empty($p->api_key);
                                @endphp
                                <label class="text-[12px] text-ink-700 block {{ ($spec['type'] ?? 'text') === 'textarea' ? 'col-span-2' : '' }}">
                                    <span class="font-mono text-[10px] uppercase tracking-[0.14em] text-ink-500">{{ $spec['label'] ?? $key }}
                                        @if (!empty($spec['required']))
                                            <span class="text-accent-coral">*</span>
                                        @endif
                                    </span>
                                    @if (($spec['type'] ?? 'text') === 'password')
                                        {{-- A stored API key is NEVER preloaded into the DOM — a value on a
                                             password input leaks the secret to anything that can read the page.
                                             Field stays empty; the controller merges blank-as-keep. --}}
                                        <input type="password"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-paper-100 pt-4">
                        <label class="text-[12px] text-ink-700 block">
                            <span class="font-mono text-[10px] uppercase tracking-[0.14em] text-ink-500">{{ __('Default model') }}</span>
                            <select name="default_model"
                                class="mt-1.5 w-full px-3 py-2 border border-paper-200 rounded-lg bg-white text-[12.5px] font-mono focus:outline-none focus:border-wa-deep focus:ring-4 focus:ring-wa-deep/10">
                                <option value="">{{ __('— Pick a default model —') }}</option>
                                @foreach ($p->model_choices ?? [] as $model)
                                    <option value="{{ $model }}" @selected($p->default_model === $model)>{{ $model }}</option>
                                @endforeach
                                @if ($p->default_model && !in_array($p->default_model, $p->model_choices ?? [], true))
                                    <option value="{{ $p->default_model }}" selected>{{ $p->default_model }} (custom)</option>
                                @endif
                            </select>
                        </label>
                        <label class="text-[12px] text-ink-700 block">
                            <span class="font-mono text-[10px] uppercase tracking-[0.14em] text-ink-500">{{ __('Sort order') }}</span>
                    ...
                    <div class="flex justify-end pt-2">
                        <button type="submit"
                            class="px-4 py-2 rounded-full ig-grad text-white text-[12.5px] font-semibold hover:opacity-95 transition">Save
                            {{ $p->name }}</button>
                    </div>
                </form>
            </section>
        @empty
            <div class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card p-12 text-center">
                ...
                <p class="text-[12.5px] text-ink-500 mt-2">{{ __('Run the migrations to seed the provider rows.') }}</p>
            </div>
        @endforelse

        {{-- Help footer card --}}
        <section class="bg-wa-mint/40 border border-wa-green/40 rounded-2xl p-5">
            ...
            <p class="text-[12.5px] text-ink-700 leading-relaxed max-w-4xl">
                {{ __('When an AI feature runs (the flow "AI" node), it uses the signed-in user\'s own key first if they set one under Account → AI keys. If none is set, it falls back to the matching Active row here. Disabling a provider above turns it off for everyone who hasn\'t brought their own key.') }}
            </p>
        </section>
