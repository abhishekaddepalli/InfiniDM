<x-layouts.admin :title="__('Admin · Announcements')" admin-key="announcements">
    ...
    <main class="px-4 sm:px-6 lg:px-7 py-7 space-y-5">
        ...
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div>
                ...
                <h1 class="font-serif font-normal tracking-[-0.01em] text-[30px] sm:text-[34px] lg:text-[40px] leading-[1.0]">{{ __('Marquee') }}
                    <span class="italic ig-text">{{ __('announcements') }}</span></h1>
                <p class="text-[13px] text-ink-600 mt-2 max-w-2xl">{{ __('Active rows chain together in a horizontal scroll above every authenticated page\'s header. Customers can dismiss individual rows when "Dismissible" is on.') }}</p>
            </div>
        ...
        <form method="get" action="{{ url()->current() }}"
            ...
            @php $statusPills = ['all' => __('All'), 'active' => __('Live now'), 'inactive' => __('Disabled'), 'expired' => __('Expired')]; @endphp
        ...
        <div class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card">
                    ...
                    @forelse ($announcements as $a)
                        ...
                        <tr class="hover:bg-paper-50/60 {{ $expired ? 'opacity-50' : '' }}">
                            ...
                            <td class="px-3 py-3 font-mono text-[11px]">
                                @if ($scheduled)
                                    <span class="text-accent-amber">{{ __('starts') }} {{ $a->starts_at->diffForHumans() }}</span>
                                @elseif ($expired)
                                    <span class="text-accent-coral">{{ __('expired') }}
                                        {{ $a->expires_at->diffForHumans() }}</span>
                                @elseif ($a->expires_at)
                                    <span class="text-ink-600">{{ __('until') }} {{ $a->expires_at->format('M j, Y') }}</span>
                                ...
                                <div class="relative inline-block" data-row-menu>
                                    ...
                                    <div data-row-menu-panel
                                        ...
                                        <form method="POST"
                                            ...
                                            onsubmit="return confirm('{{ __('Delete this announcement? It will disappear from the marquee immediately.') }}')">
                                            @csrf @method('DELETE')
                                            <button type="submit"
            ...
            <div
                ...
                <div class="text-[11px] font-mono text-ink-500">{{ __('Showing') }}
                    {{ $announcements->firstItem() ?? 0 }}–{{ $announcements->lastItem() ?? 0 }} {{ __('of') }}
