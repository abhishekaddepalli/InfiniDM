@php
    ...
    $tzCur = old('input_timezone', $a?->starts_at ? config('app.timezone') : config('app.timezone'));
@endphp
<form id="annForm" method="POST" action="{{ $action }}" class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div class="space-y-3">
            <div>
                ...
                <textarea id="an-text" name="text" rows="3" required maxlength="500"
                    class="w-full px-3 py-2 border border-paper-200 rounded-lg bg-paper-0 text-[12.5px] resize-none focus:outline-none focus:border-wa-deep focus:ring-4 focus:ring-wa-deep/10"
            ...
            <div>
                ...
                @php $tones = ['info' => __('Info (dark ink)'), 'promo' => __('Promo (brand)'), 'warning' => __('Warning (amber)'), 'success' => __('Success (green)')]; @endphp
            ...
            <div>
                ...
                <select id="an-tz" name="input_timezone"
                    ...
                    @foreach ($tzList as $tz)
                        <option value="{{ $tz }}" @selected($tzCur === $tz)>{{ $tz }}</option>
                    @endforeach
                </select>
                <div class="text-[10.5px] text-ink-500 mt-1">
                    {{ __('Reference only — the times below are stored as entered.') }}</div>
            </div>
            <div>
                <label class="text-[11.5px] font-semibold text-ink-700 mb-1.5 block"
    ...
    <div class="xl:col-span-3 flex justify-end gap-2 pt-1">
        ...
        <button type="submit"
            ...
            {{ $a ? __('Save changes') : __('Create announcement') }}
        </button>
    </div>
</form>
