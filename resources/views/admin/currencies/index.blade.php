<x-layouts.admin :title="__('Currencies')" admin-key="currencies">
    ...
    <main class="px-4 sm:px-7 py-7 space-y-5">
        ...
        <div class="flex items-end justify-between gap-4 flex-wrap">
            <div>
                ...
                <h1 class="font-serif font-normal tracking-[-0.01em] text-[40px] leading-[1.0]">{{ __('Platform') }}
                    <span class="italic ig-text">{{ __('currencies') }}</span></h1>
                <p class="text-[13px] text-ink-600 mt-2 max-w-3xl">
                    {{ __('Manage the currencies customers can pick. Exchange rates are stored relative to') }} <span
                        class="font-mono">{{ __('USD') }}</span>. {{ __('Changing the default propagates to every dashboard, invoice, and plan display.') }}</p>
            </div>
        ...
        @if (session('error'))
            <div
                class="rounded-2xl border border-accent-coral/30 bg-accent-coral/10 text-accent-coral px-4 py-2 text-[12.5px]">
                {{ session('error') }}</div>
        @endif
        @if ($errors->any())
        ...
        <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div class="bg-paper-0 border border-paper-200 rounded-2xl p-4 shadow-card">
                ...
                <div class="text-[11px] text-ink-500 mt-2">{{ __('hidden from settings') }}</div>
            </div>
        </section>
            ...
            <div class="px-5 py-4 border-b border-paper-200 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="text-[12px] text-ink-600 mt-1">
                        {{ __('Accounts without an explicit currency choice use this one. Saving here applies instantly.') }}
                    </p>
                </div>
                <form method="POST" action="{{ route('admin.currencies.default') }}"
                    ...
                    @csrf
                    <select name="code"
                        class="px-3 py-2 border border-paper-200 rounded-lg bg-paper-0 text-[12.5px] font-mono focus:outline-none focus:border-wa-deep focus:ring-4 focus:ring-wa-deep/10">
            ...
            <div class="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <label class="block">
                    ...
                    <input type="text" name="code" required maxlength="10" placeholder="{{ __('INR') }}"
                        class="mt-1.5 w-full px-3 py-2 border border-paper-200 rounded-lg bg-paper-0 text-[12.5px] font-mono uppercase focus:outline-none focus:border-wa-deep">
                </label>
                <label class="block">
                    <span
                ...
                <label class="block">
                    ...
                    <input type="number" name="precision" required min="0" max="6" value="2"
                        class="mt-1.5 w-full px-3 py-2 border border-paper-200 rounded-lg bg-paper-0 text-[12.5px] font-mono focus:outline-none focus:border-wa-deep">
                </label>
                            ...
                            <td class="px-4 py-2.5 text-right">
                                @if ($c->code !== $defaultCode)
                                    <form method="POST" action="{{ route('admin.currencies.destroy', $c->id) }}"
                                        class="inline"
                                        onsubmit="return confirm('{{ __('Delete') }} {{ $c->code }}? {{ __('Any prices saved in this currency will become orphaned.') }}')">
                                        @csrf @method('DELETE')
                                        <button
