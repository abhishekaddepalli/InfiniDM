<x-layouts.admin :title="__('Languages')" admin-key="languages">
    ...
    <main class="px-4 sm:px-6 lg:px-7 py-7 space-y-5">
        ...
        <div class="flex items-end justify-between gap-4">
            <div>
                ...
                <h1 class="font-serif font-normal tracking-[-0.01em] text-[40px] leading-[1.0]">{{ __('Platform') }}
                    <span class="italic ig-text">{{ __('languages') }}</span></h1>
                <p class="text-[13px] text-ink-600 mt-2 max-w-3xl">
                    {{ __('Manage which languages new users can pick. The "default" language is the fallback for every newly created account.') }}
                </p>
            </div>
        </div>
        ...
        <section class="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-5 items-start">
            <div class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card overflow-hidden min-w-0">
                                ...
                                <td class="px-4 py-3 text-right">
                                    @if ($l->code !== $defaultCode)
                                        <form method="POST" action="{{ route('admin.languages.destroy', $l->id) }}"
                                            class="inline" onsubmit="return confirm('{{ __('Remove') }} {{ $l->name }}?')">@csrf
                                            @method('DELETE')
                                            <button
            ...
            <aside class="space-y-4 lg:sticky lg:top-[88px]">
                <div class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card p-5">
                    ...
                    <h3 class="font-serif text-[18px] leading-tight mt-0.5 mb-2">{{ __('Managed language pack') }}</h3>
                    <p class="text-[12px] text-ink-600 leading-relaxed">
                        {{ __('Each active language is backed by a translation JSON file shipped with the platform. New language slots appear here when the translation pack is extended.') }}
                    </p>
                    <p class="text-[11.5px] text-ink-500 mt-3 leading-relaxed">
                        {{ __('You can toggle active state and pick the platform default from the table on the left.') }}
                    </p>
                </div>
