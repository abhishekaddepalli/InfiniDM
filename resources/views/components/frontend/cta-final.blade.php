...
@php
    ...
    $secondaryHref = $secondaryHref ?? url('/contact');
    $kicker = fcp('cta-final.kicker', $kicker);
    $headline = fcp(
        'cta-final.headline',
        $headline ?? 'Turn your next<br><span class="italic text-wa-green">100,000</span> DMs into sales.',
    );
...
<section class="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-7 py-28" data-fc-section="cta-final">
    <div
        class="rounded-[36px] ig-grad-cta text-paper-0 px-6 sm:px-10 lg:px-12 py-12 sm:py-16 lg:py-20 relative overflow-hidden reveal">
        ...
        <div class="relative grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
            <div class="col-span-12 lg:col-span-8">
                <div class="mono text-[10px] uppercase tracking-[0.22em] text-paper-0/60 mb-4"
                    data-fc="cta-final.kicker">{{ __($kicker) }}</div>
                <h2 class="serif text-[44px] sm:text-[64px] lg:text-[100px] leading-[0.92] tracking-[-0.02em]"
                    data-fc="cta-final.headline">
                    {!! $headline !!}
                </h2>
                <p class="text-[16px] text-paper-0/80 mt-6 max-w-xl" data-fc="cta-final.subtitle">
                    {{ __($subtitle) }}</p>

                <div class="mt-8 flex gap-3 flex-wrap">
                    <a href="{{ $primaryHref }}"
                        class="px-6 py-3.5 rounded-full bg-wa-green text-paper-0 text-[14px] font-semibold hover:bg-[#c71f5d] flex items-center gap-2 glow-green">
                        <span data-fc="cta-final.primary_label">{{ __($primaryLabel) }}</span>
                    ...
                    <a href="{{ $secondaryHref }}"
                        ...
                        data-fc="cta-final.secondary_label">{{ __($secondaryLabel) }}</a>
                </div>
            </div>
                    ...
                    <div class="mt-3 space-y-2.5">
                        <div class="flex items-center gap-2.5">
                            <span
                                ...
                                data-fc="cta-final.proof1_initials">{{ fc('cta-final.proof1_initials', 'UT') }}</span>
                            <div class="flex-1 leading-tight">
                                <div class="text-[12px] font-medium" data-fc="cta-final.proof1_name">
                                    {{ fc('cta-final.proof1_name', 'Urban Threads') }}</div>
                                <div class="mono text-[10px] text-paper-0/50" data-fc="cta-final.proof1_meta">
                                    {{ fc('cta-final.proof1_meta', 'Berlin · 12k followers') }}</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-2.5">
                            <span
                                ...
                                data-fc="cta-final.proof2_initials">{{ fc('cta-final.proof2_initials', 'CL') }}</span>
                            <div class="flex-1 leading-tight">
                                <div class="text-[12px] font-medium" data-fc="cta-final.proof2_name">
                                    {{ fc('cta-final.proof2_name', 'Café Lumen') }}</div>
                                <div class="mono text-[10px] text-paper-0/50" data-fc="cta-final.proof2_meta">
                                    {{ fc('cta-final.proof2_meta', 'Paris · migrated from ManyChat') }}</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-2.5">
                            <span
                                class="w-7 h-7 rounded-full bg-gradient-to-br from-accent-amber to-wa-green text-paper-0 text-[10px] font-semibold flex items-center justify-center"
                                data-fc="cta-final.proof3_initials">{{ fc('cta-final.proof3_initials', 'BS') }}</span>
                            <div class="flex-1 leading-tight">
                                <div class="text-[12px] font-medium" data-fc="cta-final.proof3_name">
                                    {{ fc('cta-final.proof3_name', 'Bloom Studio') }}</div>
                                <div class="mono text-[10px] text-paper-0/50" data-fc="cta-final.proof3_meta">
                                    {{ fc('cta-final.proof3_meta', 'Mumbai · 5 seats') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
