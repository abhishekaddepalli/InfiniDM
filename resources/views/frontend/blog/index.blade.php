{{--
 Public blog index — editorial layout ported from the WaDesk frontend design,
 recoloured to Instagram + wired to the standalone BlogPost (cover_image, no
 categories). Newest post headlines as a featured card; the rest fill a grid.
--}}
@extends('frontend.layout')
@section('title', __('Blog'))

@php
    // cover_image may be an uploads/ path, a storage/ path, or a full URL —
    // brand_asset() resolves the first two; a URL passes through untouched.
    $img = fn ($post) => $post && $post->cover_image
        ? (str_starts_with($post->cover_image, 'http') ? $post->cover_image : brand_asset($post->cover_image))
        : null;
    $readMins = fn ($post) => max(1, (int) ceil(str_word_count(strip_tags((string) $post->body)) / 200));
@endphp
    ...
    <section class="relative overflow-hidden bg-paper-0">
        ...
        <div class="relative max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-7 pt-24 pb-12">
            ...
            <p class="text-[15.5px] text-ink-700 leading-relaxed max-w-2xl mt-5 reveal" style="--d:120ms">
                {{ __('Insights, guides and product updates on growing your business on Instagram.') }}
            </p>
        </div>
    </section>
            ...
            <div class="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-7 pb-4">
                <a href="{{ route('blog.show', $featured->slug) }}"
                    ...
                    <div class="aspect-[16/10] lg:aspect-auto lg:h-full min-h-[280px] overflow-hidden">
                        @if ($c = $img($featured))
                            <img src="{{ $c }}" alt="{{ $featured->title }}" class="w-full h-full object-cover group-hover:scale-[1.03] transition duration-500">
                        @else
                            <div class="w-full h-full min-h-[280px] ig-grad-soft"></div>
                        @endif
                    </div>
                    <div class="p-7 lg:p-10">
                        ...
                        <h2 class="serif text-[32px] sm:text-[44px] leading-[1.0] tracking-[-0.02em] group-hover:text-wa-deep transition">{{ $featured->title }}</h2>
                        @if ($featured->excerpt)
                        ...
                        <div class="mt-6 flex items-center gap-3 text-[11.5px] mono text-ink-500">
                            ...
                            <span>{{ $readMins($featured) }} {{ __('min read') }}</span>
                        </div>
                    ...
                    @foreach ($posts as $p)
                        <a href="{{ route('blog.show', $p->slug) }}"
                            ...
                            <div class="aspect-[16/10] overflow-hidden">
                                @if ($c = $img($p))
                                    <img src="{{ $c }}" alt="{{ $p->title }}" class="w-full h-full object-cover group-hover:scale-[1.04] transition duration-500">
                                @else
                            ...
                            <div class="flex flex-col flex-1 p-6">
                                ...
                                <div class="mt-auto pt-5 flex items-center gap-2.5 text-[11px] mono text-ink-500">
                                    ...
                                    <span class="text-ink-400">·</span>
                                    <span>{{ $readMins($p) }} {{ __('min read') }}</span>
                                </div>
                            </div>
                        </a>
                    @endforeach
