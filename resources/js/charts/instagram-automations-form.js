// IgDesk automation CREATE / EDIT wizard — the multi-step stepper that
...
// in the IgDesk theme. Page keys: instagram-automations-create /
...
export default function init() {
    ...
    const applyType = (type) => {
        ...
        const keyworded = type === 'dm_keyword' || type === 'comment_to_dm' || type === 'story_reply' || type === 'story_mention';
        ...
        if (msgHint) {
            msgHint.textContent = isAi
                ...
                : 'Use {first_name} to personalise. This is the DM IgDesk sends back.';
        }
        if (trigHint) {
            const map = {
                ...
                story_mention: 'Fires when someone @-mentions you in their story — then DMs them back.',
    ...
    // ── Live DM preview + compose char-count ──
    const charCount = document.getElementById('igCharCount');
    const renderPreview = () => {
        const raw = replyText?.value || '';
        if (ppBody) ppBody.textContent = raw.trim() || 'your DM appears here…';
    ...
    const labelFor = {
        ...
        story_mention: 'Story mention', mention: 'Mention', ai_agent: 'AI agent', flow: 'Visual flow',
    };
    const fillReview = () => {
