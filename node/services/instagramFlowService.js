...
const sendButtons = (auth, igsid, text, buttons) =>
  graphSend(auth, {
    recipient: { id: igsid },
    message: {
      attachment: {
        ...
        payload: {
          ...
          buttons: buttons.slice(0, 3).map((b) => (
            String(b.type) === "web_url"
              ...
              : { type: "postback", title: String(b.title || "").slice(0, 20), payload: String(b.payload || b.title || "") }
          )),
        },
      },
    },
    ...
    try {
      switch (type) {
        ...
        case "buttons": {
          ...
          const r = await sendQuickReplies(auth, igsid, body, opts);
          await logToLaravel(appDomain, { accountId, igsid, workspaceId, direction: "out", body, source: "flow", mid: r?.message_id || null });
          park(ctx, node.id);
        ...
        case "ig_buttons": {
          const body = subst(d.text, ctx.vars);
          const r = await sendButtons(auth, igsid, body, (d.buttons || []));
          await logToLaravel(appDomain, { accountId, igsid, workspaceId, direction: "out", body, source: "flow", mid: r?.message_id || null });
          park(ctx, node.id);
          return;
        }
...
export async function resumeFlow({ accountId, igsid, text }) {
  ...
  if (!parked) { IG_SESSIONS.delete(key); return false; }
