...
/**
 * Four ready-to-test demo flows for the connected Instagram account, one per
 * feature the operator asked to try:
 *   menu   → quick-reply BUTTONS with two branches
 *   links  → CTA / URL buttons (generic template)
 *   signup → ASK questions that SAVE answers to contact attributes
 *   hi     → a welcome that PRE-FILLS {{name}} from a returning contact
 *
 * Seeded through the model so flow_data is encrypted and the managed
 * instagram_automations row is created by the saved() hook — identical to a
 * flow authored in the builder. Idempotent by (user_id, flow_name).
 */
return new class extends Migration
{
    public function up(): void
    {
        // Prefer admin@mediacity.co.in's connected account; fall back to any.
        $account = null;
        try {
            $userId = optional(\App\Models\User::where('email', 'admin@mediacity.co.in')->first())->id;
            if ($userId) {
                $account = InstagramAccount::where('user_id', $userId)
                    ->where('status', 'connected')->orderBy('id')->first();
            }
        } catch (\Throwable $e) {
            // Users table shape differs — fall through to the generic pick.
        }
        $account = $account ?: InstagramAccount::where('status', 'connected')->orderBy('id')->first();
        if (!$account) return; // nothing connected — nothing to seed
        ...
        $flows[] = [
            ...
            'nodes' => [
                ...
                $node('n_cta', 'ig_buttons', ['text' => 'Explore us:', 'buttons' => [
                    ['type' => 'web_url', 'title' => 'Visit site', 'url' => 'https://templatecookies.com'],
                    ['type' => 'web_url', 'title' => 'Shop now',   'url' => 'https://templatecookies.com/instaflow/public'],
                ]]),
            ],
            'edges' => [
        ...
        foreach ($flows as $f) {
            $exists = InstagramFlow::where('user_id', $userId)->where('flow_name', $f['name'])->exists();
            if ($exists) continue;
    ...
    {
        try {
            InstagramFlow::whereIn('flow_name', [
                'Demo — Buttons (menu)',
                'Demo — CTA links (links)',
                'Demo — Attributes (signup)',
                'Demo — Welcome pre-fill (hi)',
            ])->get()->each->delete(); // each->delete fires the deleted() hook → cleans automations
        } catch (\Throwable $e) {
            // best effort
        }
    }
