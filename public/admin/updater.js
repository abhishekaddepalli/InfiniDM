/* InstaMagic admin Updater — drives verify → backup → upload → apply → migrate →
...
(function () {
  ...
  function banner(text, ok) {
    ...
    if (!b) return;
    b.textContent = text;
    b.className = 'rounded-2xl px-4 py-3 text-[13px] font-medium border ' +
      (ok ? 'bg-wa-bubble text-wa-deep border-wa-green/40'
  ...
  function setMsg(n, text, ok) {
    ...
    m.textContent = text;
    m.className = 'mt-2 text-[12px] font-medium ' + (ok ? 'text-wa-deep' : 'text-accent-coral');
  }
  ...
  function renderHealth(health) {
    ...
    Object.keys(health).forEach(function (k) {
      ...
      d.className = 'flex items-center gap-1.5 font-medium ' + (ok ? 'text-wa-deep' : 'text-accent-coral');
  ...
  root.querySelectorAll('[data-rollback]').forEach(function (el) {
    ...
    el.addEventListener('click', function () {
      if (!armed) {
        ...
        el.classList.add('bg-accent-coral', 'text-paper-0');
        timer = setTimeout(function () {
          armed = false; el.textContent = 'Rollback'; el.classList.remove('bg-accent-coral', 'text-paper-0');
        }, 4000);
        return;
      }
      ...
      post(CFG.urls.rollback, { backup_dir: el.getAttribute('data-rollback') }).then(function (res) {
        ...
        else { el.disabled = false; el.textContent = 'Rollback'; el.classList.remove('bg-accent-coral', 'text-paper-0'); }
      }).catch(function () {
        el.disabled = false; el.textContent = 'Rollback'; el.classList.remove('bg-accent-coral', 'text-paper-0');
