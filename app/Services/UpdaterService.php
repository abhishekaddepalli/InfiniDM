 ...
 * code → migrate → finalize, with rollback. Extended with an Envato
 * purchase-code verification step. NEVER touches .env, storage, the database
 * files, vendor/ or user-uploaded public assets.
 */
class UpdaterService
{
    ...
    private const PROTECTED_PATHS = [
        ...
        // Node runtime data that MUST survive an update: its own env file and
        // installed deps. The root entries above only match the root subtree
        // (path-prefix), so the node/ equivalents are listed explicitly — a
        // mis-packaged update ZIP must never overwrite node secrets.
        'node/.env', 'node/node_modules',
    ];
    ...
    private const UPDATABLE_PATHS = [
        ...
        'public/css', 'public/js', 'public/build', 'public/extensions',
    ...
    public function verifyPurchase(string $code): array
    {
        ...
        Setting::set('envato_purchase_code', $code, 'string');
        Setting::set('envato_verified_at', now()->toIso8601String(), 'string');
    ...
    public function createBackup(): array
    {
        ...
        // The shutdown handler captures a PHP FATAL (memory/timeout) inside the DB
        // dump — the only hook that still runs after PHP aborts the request. Grep
        // the log for "[UPDATER-BACKUP]".
    ...
    /**
     * Log-only fatal capture for the backup step. A memory/timeout fatal is NOT
     * catchable by try/catch, so without this the request just dies and the log
     * stays empty. Registered once per request; changes no behaviour.
     */
    ...
    private function isProtected(string $relPath): bool
    {
        ...
        if (str_starts_with($relPath, 'public/')) {
            $allowedCodeDirs = ['public/css/', 'public/js/', 'public/build/', 'public/extensions/'];
            $inCodeDir = false;
     ...
     * update back, and a huge `notifications` table can exhaust memory mid-dump
     * and abort the request with a PHP FATAL that no try/catch could catch. The
     * CREATE TABLE is still written, so a restore rebuilds them empty.
     */
    private const BACKUP_SKIP_ROWS = [
        'notifications',      // UI feed — grows unbounded
    ...
    private function dumpDatabasePhp(string $path): void
    {
        ...
        // STREAM to disk. Writing row-by-row keeps memory flat regardless of
        // database size.
        $fh = @fopen($path, 'w');
        if ($fh === false) {
        ...
        fwrite($fh, "-- InstaMagic Database Backup\n-- Date: " . now()->toIso8601String() . "\n\n");
