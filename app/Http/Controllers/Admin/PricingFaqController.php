...
/**
 * Admin pricing-FAQ CRUD — manages the accordion shown on /instagram/plans.
 * List + inline create/edit + toggle + delete, matching the other admin lookups.
 */
class PricingFaqController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data['sort_order'] = $data['sort_order'] ?? ((int) PricingFaq::max('sort_order') + 1);
        PricingFaq::create($data);
        return back()->with('success', __('FAQ created.'));
    }
    ...
    public function toggle(int $id)
    {
        ...
        return back()->with('success', $faq->is_active ? __('Activated.') : __('Deactivated.'));
    }

    public function destroy(int $id)
    {
        PricingFaq::findOrFail($id)->delete();
        return back()->with('success', __('FAQ removed.'));
    }
