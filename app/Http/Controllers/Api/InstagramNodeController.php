...
class InstagramNodeController extends Controller
{
    ...
    {
        // Same source as the flow-node bridge: admin "Node token" wins, then
        // config/env. Keeps every Node endpoint on ONE token so the operator
        // configures it in one place (admin → Instagram settings → Node token).
        $expected = (string) (\App\Services\Instagram\InstagramGate::nodeToken()
            ?: config('services.node.token')
            ?: env('NODE_WEBHOOK_TOKEN', ''));
    ...
    {
        $v    = (string) InstagramGate::setting('instagram_graph_version', 'v21.0');
        $host = $a->login_type === 'instagram' ? 'graph.instagram.com' : 'graph.facebook.com';
        // Node's IgGraphClient destructures `igUserId`; ship BOTH keys so the
        // send URL is `{base}/{ig-id}/messages` and never `{base}//messages`
        // (empty id → Graph error 100 "Object with ID 'messages' does not exist").
        return [
            'base'      => "https://{$host}/{$v}",
            'token'     => (string) $a->access_token,
            'ig'        => (string) $a->ig_user_id,
            'igUserId'  => (string) $a->ig_user_id,
        ];
    }
    ...
    public function due(Request $r): JsonResponse
    {
        ...
        foreach (InstagramScheduledPost::where('status', 'pending')
            ...
            // Content protection — Node posts these URLs straight to Meta, so we
            // watermark HERE and hand Node the watermarked copies. Fail-safe: no
            // active config / GD missing / any error → the original URLs (see
            // InstagramController::applyWatermark + WatermarkService::apply).
            $wmData = \App\Http\Controllers\InstagramController::applyWatermark($a, (string) ($p->media_type ?: 'image'), [
                'image_url'  => $p->image_url,
                'media_urls' => is_array($p->media_urls) ? $p->media_urls : null,
            ]);
            ...
            $scheduled[] = [
                ...
                'image_url'  => $wmData['image_url'] ?? $p->image_url,
                'video_url'  => $p->video_url,
                'media_urls' => $wmData['media_urls'] ?? $p->media_urls,
                'caption'    => $p->caption,
     ...
     */
    /**
     * POST /api/instagram/refresh-tokens — Node-triggered long-lived token
     * keep-alive. Node owns the TIMING (calls this on its tick); the exchange
     * runs HERE because tokens are encrypted at rest and the FB extend needs
     * the app secret. Re-exchanges any connected account within 7 days of
     * expiry so accounts never silently flip to needs_reauth.
     */
    public function refreshTokens(Request $r): JsonResponse
    {
        $this->assertNode($r);
        $accounts = InstagramAccount::where('status', 'connected')
            ->where(function ($w) {
        ...
        foreach ($accounts as $account) {
            try {
                ...
                $res = $account->login_type === 'instagram'
                    ...
                    : \App\Services\Instagram\InstagramService::extendFacebookToken((string) $account->access_token);
        ...
        Log::info('[IG-TOKEN] node-triggered refresh', ['refreshed' => $ok, 'failed' => $fail, 'checked' => $accounts->count()]);
        return response()->json(['ok' => true, 'refreshed' => $ok, 'failed' => $fail, 'checked' => $accounts->count()]);
    }
    ...
    public function claim(Request $r): JsonResponse
    {
        ...
        // instagram_repost_items has NO node_job_id column (only the scheduled /
        // broadcast tables do) — including it here threw "unknown column" → 500,
        // so a reposter claim never succeeded and queued reels never posted.
        $update = ['claimed_at' => now()];
        if ($table !== 'instagram_repost_items') {
            $update['node_job_id'] = $job;
        }

        $won = DB::table($table)->where('id', $id)->whereNull('claimed_at')->update($update);
    ...
    public function result(Request $r): JsonResponse
    {
        ...
        if ($kind === 'repost') {
            ...
            if ($status === 'posted' && $media !== '') {
                ...
                // Stamp the post cursor so the "Last post" pill stops lying.
                \App\Models\InstagramReposterSetting::where('instagram_account_id', $it->instagram_account_id)
    ...
    public function enqueue(Request $r): JsonResponse
    {
        ...
        // Stamp the scrape cursor for THIS account — a clip reaching enqueue means
        // the worker just scraped, so the "Last scrape" pill reflects reality even
        // when every clip turns out to be a duplicate below.
        \App\Models\InstagramReposterSetting::where('instagram_account_id', $account->id)
            ->update(['last_scrape_at' => now()]);
        ...
        // function_exists, like the other media_disk() call sites in this
        // package — the helper is the host's, and standalone falls back to
        // the local `public` disk it ships with.
        $path = $r->file('video')->store('ig-reposter', function_exists('media_disk') ? media_disk() : 'public');
    ...
    public function cleanup(Request $r): JsonResponse
    {
        ...
        InstagramRepostItem::where('instagram_account_id', $accountId)
            ...
            ->chunkById(100, function ($items) use (&$n) {
                foreach ($items as $it) {
                    // media_storage() is core-only; standalone falls back to the
                    // same disk media_disk() picks for uploads, else the swallowed
                    // undefined-function Error orphans the file on disk.
                    try {
                        if (function_exists('media_storage')) {
                            media_storage()->delete($it->video_path);
                        } else {
                            \Illuminate\Support\Facades\Storage::disk(function_exists('media_disk') ? media_disk() : 'public')
                                ->delete($it->video_path);
                        }
                    } catch (\Throwable $e) {}
