...
/**
 * Audit ledger for wallet movements. A WalletService (not part of this billing
 * foundation phase) would be the only thing that should `create()` rows here —
 * direct callers will skip the users balance mirror update and the two will
 * diverge.
 *
 * `kind` separates two parallel balances:
 *  - 'credit'   = message/usage credits
 *  - 'currency' = paid top-ups in the wallet currency (stored in minor units)
 *
