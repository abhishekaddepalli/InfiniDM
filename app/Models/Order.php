...
/**
 * Plan-purchase order (a SaaS subscription/plan checkout — NOT the Instagram
 * storefront order, which is App\Models\InstagramOrder). Created at
 * /checkout/{package_id}, mutated by the gateway driver during initiate(), and
 * marked paid/failed by the callback or webhook.
 *
 * Statuses: pending | paid | failed | refunded
 *
 * INSTAFLOW NOTE: WaDesk keyed orders to a Workspace model. IgDesk ships no
 * Workspace model — workspace_id is kept as a plain column (nullable) for
 * tenant scoping, but there is no workspace() relation. The buyer is the user.
 */
class Order extends Model
{
    protected $fillable = [
        ...
        // End-to-end checkout fields — admin-controlled pricing / tax / coupons
        // make their way into the order row.
    ...
    /**
     * Generate a human-readable order number. Format: IFL-{ymd}-{6char}.
     */
    public static function generateOrderNumber(): string
    {
        do {
            $candidate = 'IFL-' . now()->format('ymd') . '-' . strtoupper(\Illuminate\Support\Str::random(6));
